...
const char* YawCorrectorMPEG2::GetKernelSource() const {
  return R"(
__kernel void yaw_correction(__global const uchar* input,
    ...
    // UV: MPEG2/top-left sampling
    if (x < width/2 && y < height/2) {
        int uv_width = width / 2;
        int uv_height = height / 2;
        int y_size = width * height;
        float uv_longitude = (2.0f * (x+0.0f) / (float)uv_width - 1.0f) * PI;
        float uv_latitude = (1.0f - 2.0f * (y+0.0f) / (float)uv_height) * (PI / 2.0f);
